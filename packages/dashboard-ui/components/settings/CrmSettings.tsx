...
export function CrmSettings() {
  ...
  const onSubmit = (data: FormData) => {
    createMutation.mutate({
      ...
      apiUrl: data.apiUrl,
      apiKey: data.apiKey,
      webLink: data.webLink?.trim() || undefined,
      csatLink: data.csatLink?.trim() || undefined,
      config: {
        provider: "meta",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Connect to your external CRM to sync contacts and analyze messaging
            campaigns.
          </p>
        </div>
        {!isAdding && (
      ...
      {isAdding && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Webview link (optional)
              </label>
              <input
                {...register("webLink")}
                placeholder="https://crm.example.com/webview"
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[var(--primary)] focus:ring-[var(--primary)] dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm py-2 px-3"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Link to this CRM&apos;s webview base pages (for sending users to
                webview)
              </p>
              {errors.webLink && (
                <p className="text-red-500 text-xs mt-1">
            ...
            <div>
              ...
              <input
                {...register("csatLink")}
                placeholder="Leave blank to use Webview link + /csat"
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[var(--primary)] focus:ring-[var(--primary)] dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm py-2 px-3"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                URL sent to the user when a chat is resolved. If not set, uses
                Webview link + /csat.
              </p>
              {errors.csatLink && (
                <p className="text-red-500 text-xs mt-1">
                  {errors.csatLink.message}
                </p>
              )}
            </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {integration.webLink && (
                      <div className="text-xs text-gray-400 mt-0.5">
                        Webview: {integration.webLink}
                      </div>
                    )}
                    {integration.csatLink && (
                      <div className="text-xs text-gray-400 mt-0.5">
                        CSAT: {integration.csatLink}
                      </div>
                    )}
                    {integration.config?.phoneNumber && (
                      <div className="text-xs text-gray-400 mt-0.5">
                        WA: {integration.config.phoneNumber}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">

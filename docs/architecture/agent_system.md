...
- **AssignmentService**: Implements routing logic.
  ...
  - **Max load**: Per-team `routingConfig.maxLoad` caps how many concurrent chats an agent can have; agents at or above the limit are excluded from new assignments.
  - **Queue assignment**: Unassigned sessions can be assigned automatically when an agent goes online, or on demand via `POST /agent/inbox/assign-queue`.
- **Controllers**:

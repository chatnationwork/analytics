...
describe('Tenant Level Routing Test', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(InboxSessionEntity), useValue: { save: jest.fn(s => s), findOneOrFail: jest.fn() } },
        { provide: getRepositoryToken(TeamEntity), useClass: Repository },
        { provide: getRepositoryToken(TeamMemberEntity), useClass: Repository },
        { provide: getRepositoryToken(AgentProfileEntity), useClass: Repository },
        { 
    ...
    tenantMembershipRepo = module.get(getRepositoryToken(TenantMembershipEntity));
    ...
    jest.spyOn(tenantMembershipRepo, 'find').mockImplementation(async ({ where }: any) => {
  ...
  it('should find agents with MEMBER role when no Team ID is provided', async () => {
    ...
    (service as any).getAgentDetails = jest.fn().mockResolvedValue([
